...
import AttachmentEditorPopover from './AttachmentEditorPopover';
...
function fmtBudgetMonth(idx) {
  return idx != null && idx >= 0 && idx <= 11 ? MONTHS[idx] : '';
}

function AttachmentStatusPill({ attachment, onClick }) {
  const missing = attachment.status === 'missing';
  return (
    <button
      type="button"
      onClick={onClick}
      className={`inline-flex items-center gap-1 rounded-full px-2 py-1 cursor-pointer hover:brightness-95 ${missing ? 'bg-red-50 text-red-700' : 'bg-primary-light text-primary'}`}
      title={missing ? 'File missing — click to retry preview' : 'Open preview'}
    >
      <span className="material-symbols-outlined" style={{ fontSize: '14px' }}>
        {missing ? 'warning' : 'attach_file'}
      </span>
      {missing ? 'Missing' : 'Attached'}
    </button>
  );
}

function AttachmentCell({ tx, editable = false, onOpen, onRemove, onAttach, onToast }) {
  ...
  if (!tx.attachment) {
    if (!editable) return <span className="text-on-surface-tertiary">—</span>;
    return (
      <span className="relative inline-flex">
        <button
          ref={buttonRef}
          type="button"
          title="Attach a file (link if inside attachment root, upload otherwise)"
        ...
        {popoverOpen && (
          <AttachmentEditorPopover
            anchorRef={buttonRef}
            onAttach={(payload) => onAttach?.(tx.row, payload)}
  ...
  if (!editable) {
    return <AttachmentStatusPill attachment={tx.attachment} onClick={handleOpen} />;
  }

  return (
    <span className="inline-flex items-center gap-1">
      <AttachmentStatusPill attachment={tx.attachment} onClick={handleOpen} />
      <button
        type="button"
        onClick={async () => {
          if (!window.confirm('Remove the attachment from this transaction?')) return;
          const deleteFile = window.confirm('Also delete the physical file from disk? Click Cancel to remove only the link.');
          try {
            await onRemove?.(tx.row, { deleteFile });
...
function SkeletonRow() {
  return (
    <tr className="border-b border-surface-border">
      ...
      <td className="px-3 py-2.5"><div className="skeleton h-4 w-12" /></td>
      <td className="px-3 py-2.5"><div className="skeleton h-4 w-16" /></td>
      <td className="px-3 py-2.5"><div className="skeleton h-4 w-16" /></td>
      <td className="px-3 py-2.5"><div className="skeleton h-4 w-16" /></td>
      <td className="px-3 py-2.5"><div className="skeleton h-4 w-20" /></td>
...
export default function TransactionTable({
        ...
        <table className="min-w-full">
          <thead>
            <tr className="border-b border-surface-border bg-surface-dim">
              {['Date', 'Type', 'Recipient', 'Notes', 'IBAN', 'Inflow', 'Outflow', 'Balance', 'Lux CF Category', 'Budget Category', 'Budget Month', 'Updated', 'Document', 'Invoice', ''].map((h, i) => (
  ...
  const startEdit = (tx, event) => {
    ...
    setEditData({
      ...
      budgetMonth: tx.budgetMonth ?? monthIndexFromDate(tx.date) ?? 0,
  ...
  const handleChange = (field, value) => {
    ...
    setEditData((prev) => {
      ...
      // Only an inflow settles a receivable, so a selected invoice cannot
      // survive the amount moving to the outflow column.
      if ((field === 'inflow' || field === 'outflow') && next.invoiceNumber && !(Number(next.inflow) > 0)) {
        next.invoiceNumber = '';
      }
      return next;
    });
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full">
          ...
          <thead>
            ...
            <tr className="border-b border-surface-border bg-surface-dim">
              {[
                ...
                { key: 'budgetMonth', label: 'Budget Month', align: 'left' },
          ...
          <tbody>
              ...
              if (isEditing) {
                return (
                  <tr key={tx.row} ref={editRowRef} className="bg-primary-light border-b border-surface-border">
                    ...
                    <td className="px-2 py-1.5">
                      <select
                        value={editData.budgetMonth ?? ''}
                        onChange={(e) => handleChange('budgetMonth', Number(e.target.value))}
                        className={inputClass}
                        disabled={!editData.budgetCategory}
                        title={!editData.budgetCategory ? 'Select a budget category first' : undefined}
                      >
                        {MONTHS.map((m, idx) => (
                    ...
                    <td className="px-2 py-1.5">
                      {(() => {
                        const isInflow = Number(editData.inflow) > 0;
                        const linked = editData.invoiceNumber;
                        // The linked invoice is paid, so it is absent from the
                        // open list — keep it selectable or saving would silently
                        // unlink it.
                        const linkedMissing = linked && !(openInvoices || []).some((i) => i.invoiceNumber === linked);
                        const mismatch = describeInvoiceAmountMismatch(
                          findInvoiceByNumber(openInvoices, linked),
                          editData.inflow,
                        );
                        return (
                          <>
                            <select
                              ...
                              className={inputClass}
                              disabled={!isInflow}
                              title={!isInflow ? 'Only an inflow transaction can settle an invoice' : undefined}
                            >
                              <option value="">-- None --</option>
                              {linkedMissing && <option value={linked}>{linked}</option>}
                                ...
                                <optgroup key={group.year} label={group.year}>
                                  {group.invoices.map((inv) => (
                                    <option key={inv.invoiceNumber} value={inv.invoiceNumber}>
                                      {`${inv.invoiceNumber} · ${inv.recipient}`}
                                    </option>
                                  ))}
                                </optgroup>
                              ))}
                            ...
                            {mismatch && (
                              <span className="mt-1 flex items-center gap-0.5 text-xs text-amber-600" title={mismatch}>
            ...
            const rowBg = tx.checked ? 'bg-emerald-50' : 'bg-white';
            const rowHoverBg = tx.checked ? 'group-hover:bg-emerald-100' : 'group-hover:bg-surface-dim';
            const checkTitle = tx.checked
              ? `Checked${tx.checkSource === 'pdf' ? ' via statement import' : ''}${tx.checkedAt ? ' — ' + fmtTs(tx.checkedAt) : ''}. Click to uncheck.`
              : 'Mark as seen on the bank statement';
            return (
              <tr
                key={tx.row}
                className={`group border-b border-surface-border ${rowBg} ${rowHoverBg} transition-colors`}
              >
                  <td className={`px-3 py-2 text-sm whitespace-nowrap text-on-surface sticky left-0 z-10 ${rowBg} ${rowHoverBg} transition-colors`}>
                    <div className="flex items-center gap-2">
                      <input
                        type="checkbox"
                        checked={!!tx.checked}
                        onClick={(e) => e.stopPropagation()}
                        onChange={(e) => onToggleChecked?.(tx.row, e.target.checked)}
                        title={checkTitle}
                  ...
                  <td className="px-3 py-2 text-xs text-on-surface-tertiary whitespace-nowrap" onClick={(e) => e.stopPropagation()}>
                    <AttachmentCell tx={tx} onOpen={onOpenAttachment} onToast={onToast} />
                  </td>
                  <td className="px-3 py-2 text-xs whitespace-nowrap">
                    {tx.invoiceNumber ? (
                      <span className="inline-flex items-center gap-1 rounded-full px-2 py-1 bg-primary-light text-primary" title={`Settles invoice ${tx.invoiceNumber}`}>
