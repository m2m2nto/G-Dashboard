...
export default function TransactionImpactDialog({ open, data, isUpdate, budgetImpact, onBudgetEntryChange, onConfirm, onCancel, submitting }) {
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-start gap-3 bg-accent-light/50 rounded-xl p-3">
                ...
                <div className="text-sm">
                  ...
                  {budgetCat ? (
                    <div className="text-on-surface-secondary mt-0.5 space-y-2">
                      <div>
                        Mapped to budget category <span className="font-medium">{budgetCat}</span> (row {data.budgetRow})
                        {budgetImpact?.monthLabel && <> in <span className="font-medium">{budgetImpact.monthLabel}</span></>}
                      </div>
                      {budgetImpact && (
                        <div className="grid grid-cols-3 gap-2 text-xs bg-white/70 rounded-lg p-2">
                          <div>
                            <div className="text-on-surface-tertiary">Current</div>
                            <div className="font-medium text-on-surface">{fmt(budgetImpact.currentTotal)}</div>
                          </div>
                          <div>
                            <div className="text-on-surface-tertiary">Add</div>
                            <div className="font-medium text-on-surface">{fmt(budgetImpact.amount)}</div>
                          </div>
                          <div>
                            <div className="text-on-surface-tertiary">Predicted</div>
                            <div className="font-semibold text-on-surface">{fmt(budgetImpact.predictedTotal)}</div>
                          </div>
                        </div>
                      )}
                      {budgetImpact && (
                        <label className="flex items-start gap-2 text-xs text-on-surface-secondary">
                          <input
                            type="checkbox"
                            className="mt-0.5 rounded border-surface-border"
                            checked={data.createBudgetEntry !== false}
                            onChange={(e) => onBudgetEntryChange?.(e.target.checked)}
                          />
                          <span>
                            Add/update linked budget entry
                            {budgetImpact.hasExistingValue && data.createBudgetEntry === false && (
                              <span className="block text-on-surface-tertiary">Skipped to avoid duplicating a value already present in this budget cell.</span>
                            )}
                          </span>
                        </label>
                      )}
