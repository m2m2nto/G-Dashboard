...
import { applyActivityFilters } from './activityFilters.js';
import { buildBudgetImpact, shouldCreateBudgetEntryByDefault } from './budgetImpact.js';
import { findInvoiceByNumber } from './invoiceLinkHints.js';
import {
  getTransactions,
  getTransactionYears,
...
function buildAttachmentRelativePath({ date, recipient, originalFileName }) {
  ...
  const baseName = sanitizeAttachmentPathSegment(`${dateDigits} - ${safeRecipient}`);
...
export default function App() {
  ...
  const [reconcile, setReconcile] = useState(null); // { loading, report } | null
  ...
  const [invoiceForm, setInvoiceForm] = useState(null); // { mode, initial } | null
  const [invoiceSubmitting, setInvoiceSubmitting] = useState(false);
  const [invoiceDelete, setInvoiceDelete] = useState(null); // invoice | null
  const [invoiceMarkPaid, setInvoiceMarkPaid] = useState(null); // invoice | null
  const [openInvoices, setOpenInvoices] = useState([]); // unpaid, across all invoice years
  ...
  const disabledSections = useMemo(() => {
    ...
    if (!invoiceYears.includes(globalYear)) disabled.add('invoices');
    if (cfYears.length === 0) disabled.add('analytics');
    return disabled;
  }, [globalYear, txYears, cfYears, budgetYears, invoiceYears]);
  ...
  const initApp = useCallback(() => {
    ...
    getInvoiceYears().then(setInvoiceYears).catch(() => {});
  ...
  const loadCashFlow = useCallback(async () => {
    setCfLoading(true);
    try {
      // Always attempt the sync: the server skips it (without touching the CF
      // file) when there is no transaction file for the year.
  ...
  const loadInvoices = useCallback(async () => {
    ...
    try {
      const { invoices: rows, summary } = await getInvoices(globalYear);
      setInvoices(rows);
      setInvoiceSummary(summary);
    } catch (err) {
      pushToast('error', 'Failed to load invoices: ' + err.message);
    }
    setInvoicesLoading(false);
  }, [globalYear, pushToast]);

  useEffect(() => {
    if (section === 'invoices' && !disabledSections.has('invoices')) loadInvoices();
  }, [section, globalYear, disabledSections, loadInvoices]);

  // Unpaid invoices across ALL registered years — a January payment settles a
  // December invoice, and each entry carries the year of the workbook it lives
  // in. Independent of globalYear for that reason.
  const loadOpenInvoices = useCallback(async () => {
    try {
      const { invoices: rows } = await getOpenInvoices();
      setOpenInvoices(rows || []);
    } catch (err) {
      pushToast('error', 'Failed to load open invoices: ' + err.message);
    }
  }, [pushToast]);

  useEffect(() => {
    if (section === 'cashflow' && cfView === 'transactions') loadOpenInvoices();
  }, [section, cfView, loadOpenInvoices]);

  const invoiceRecipients = useMemo(
    () => [...new Set(invoices.map((i) => i.recipient).filter(Boolean))].sort(),
    [invoices]
  );
  ...
  const handleNewInvoice = useCallback(async () => {
    let invoiceNumber = '';
    try {
      ({ invoiceNumber } = await getNextInvoiceNumber(globalYear));
    } catch {}
    setInvoiceForm({ mode: 'add', initial: { invoiceNumber } });
  }, [globalYear]);

  const handleEditInvoice = useCallback((inv) => {
    setInvoiceForm({ mode: 'edit', initial: { ...inv } });
  }, []);

  const handleSubmitInvoice = useCallback(async (data) => {
    setInvoiceSubmitting(true);
    try {
      if (invoiceForm?.mode === 'edit') {
        await updateInvoice(globalYear, data.row, data);
        pushToast('success', `Invoice ${data.invoiceNumber} updated`);
      } else {
        await addInvoice(globalYear, data);
        pushToast('success', `Invoice ${data.invoiceNumber} added`);
      }
      setInvoiceForm(null);
      await loadInvoices();
    } catch (err) {
  ...
  }, [invoiceForm, globalYear, pushToast, loadInvoices]);

  const confirmMarkPaidInvoice = useCallback(async (paymentDate) => {
    if (!invoiceMarkPaid) return;
    setInvoiceSubmitting(true);
    try {
      await updateInvoice(globalYear, invoiceMarkPaid.row, { ...invoiceMarkPaid, paymentDate });
      pushToast('success', `Invoice ${invoiceMarkPaid.invoiceNumber} marked paid`);
      setInvoiceMarkPaid(null);
      await loadInvoices();
    } catch (err) {
      pushToast('error', err.message);
    }
    setInvoiceSubmitting(false);
  }, [invoiceMarkPaid, globalYear, pushToast, loadInvoices]);

  const handleAttachInvoice = useCallback(async (inv) => {
    try {
      const { attachment } = await selectInvoiceAttachment(globalYear, inv.invoiceNumber);
      if (attachment) {
        pushToast('success', `Linked ${attachment.fileName} to ${inv.invoiceNumber}`);
        await loadInvoices();
      }
    } catch (err) {
      pushToast('error', err.message);
    }
  }, [globalYear, pushToast, loadInvoices]);

  const handleOpenInvoiceAttachment = useCallback(async (inv) => {
    try {
      await openInvoiceAttachment(globalYear, inv.invoiceNumber);
    } catch (err) {
      pushToast('error', err.message);
    }
  }, [globalYear, pushToast]);

  const handleRemoveInvoiceAttachment = useCallback(async (inv) => {
    try {
      await removeInvoiceAttachment(globalYear, inv.invoiceNumber);
      await loadInvoices();
    } catch (err) {
      pushToast('error', err.message);
    }
  ...
  const confirmDeleteInvoice = useCallback(async () => {
    if (!invoiceDelete) return;
    try {
      await deleteInvoice(globalYear, invoiceDelete.row);
      pushToast('success', `Invoice ${invoiceDelete.invoiceNumber} deleted`);
  ...
  }, [invoiceDelete, globalYear, pushToast, loadInvoices]);
  ...
  const loadBudgetEntriesForImpact = async () => {
    const data = await getBudgetEntries(globalYear);
  ...
  const preparePendingTransaction = async (data, row = null) => {
    const entries = await loadBudgetEntriesForImpact().catch(() => budgetEntries || []);
    const originalKey = row != null ? `${month}-${row}` : null;
    const budgetImpact = buildBudgetImpact({
      entries,
      transaction: data,
      originalTransactionKey: originalKey,
    });
    const dataWithBudgetChoice = {
      ...data,
      createBudgetEntry: data.createBudgetEntry ?? shouldCreateBudgetEntryByDefault(budgetImpact),
    };
    setPendingTx({ data: dataWithBudgetChoice, row, budgetImpact });
  };

  const handleUpdateTransaction = async (row, data) => {
    await preparePendingTransaction(data, row);
  };

  const handleDeleteTransaction = async (row) => {
    const settledInvoice = transactions.find((t) => t.row === row)?.invoiceNumber || null;
    await deleteTransaction(globalYear, month, row);
    // Deleting the payment drops the link but deliberately does not un-pay the
    // invoice — say so, or the invoice quietly stays paid with nothing paying it.
    if (settledInvoice) {
      pushToast('info', `Invoice ${settledInvoice} is still marked paid — clear its payment date in Invoices if that is wrong.`);
    }
  ...
  const handleToggleChecked = async (row, checked) => {
    // Optimistic: flip locally, then persist; revert on failure.
    setTransactions((prev) => prev.map((t) => (t.row === row ? { ...t, checked, checkSource: 'manual' } : t)));
    try {
      await setTransactionChecked(globalYear, month, row, checked);
    } catch (err) {
      setTransactions((prev) => prev.map((t) => (t.row === row ? { ...t, checked: !checked } : t)));
      pushToast('error', err.message || 'Unable to update checked state.');
    }
  };

  const handlePickStatement = () => statementInputRef.current?.click();

  const handleStatementSelected = async (event) => {
    const file = event.target.files?.[0];
    event.target.value = ''; // allow re-selecting the same file
    if (!file) return;
    setReconcile({ loading: true, report: null });
    try {
      const report = await importBankStatement(globalYear, month, file);
      setReconcile({ loading: false, report });
    } catch (err) {
      setReconcile(null);
      pushToast('error', err.message || 'Unable to read the statement.');
    }
  };

  const handleApplyReconciliation = async (rows) => {
    try {
      if (rows.length) {
        const result = await applyReconciliation(globalYear, month, rows);
        pushToast('success', `Checked ${result.checked} transaction${result.checked === 1 ? '' : 's'} from the statement.`);
      }
      setReconcile(null);
      await loadTransactions({ silent: true });
    } catch (err) {
      pushToast('error', err.message || 'Unable to apply reconciliation.');
    }
  };
  ...
  const handleAttachFileForRow = async (row, { relativePath, absolutePath, destinationFolder } = {}) => {
    const result = await attachTransactionFile(globalYear, month, row, { relativePath, absolutePath, destinationFolder });
    await loadTransactions({ silent: true });
    return result;
  };

  const handleAddTransaction = async (formData) => {
    await preparePendingTransaction(formData);
    return true;
  };

  const handleConfirmTransaction = async () => {
    if (!pendingTx) return;
    const { data, row, budgetImpact } = pendingTx;
    const { attachmentPick, destinationFolder, budgetMonth, budgetMonthManual, createBudgetEntry, invoiceNumber, ...transactionData } = data;
    ...
    try {
      if (isUpdate) {
        result = await updateTransaction(globalYear, month, row, transactionData);
      ...
      // Sync the linked budget entry (1-1 with transaction)
      {
        ...
        // On update: if the server moved the row to a new sheet, use the new row;
        // otherwise the URL row is still correct.
        const txRow = isUpdate ? (result?.moved && result?.row != null ? result.row : row) : result?.row;
        const txKey = txRow != null ? `${txMonthName}-${txRow}` : null;
        const originalKey = isUpdate ? `${month}-${row}` : null;
        const amount = Number(transactionData.inflow) || Number(transactionData.outflow) || 0;

        try {
          // Find existing linked entry. On move, the entry is still keyed by
          // the pre-update (month, row) — look that up first so we re-key it
          // instead of orphaning the original and creating a duplicate.
          const findExisting = (key) => (key && budgetEntries
            ? budgetEntries.find((e) => e.transactionKey === key)
            : null);
          const existing = findExisting(originalKey) || findExisting(txKey);

          if (transactionData.budgetCategory && transactionData.budgetRow != null && amount && createBudgetEntry !== false) {
            const entryData = {
            ...
            if (budgetMonth != null) entryData.competencyMonth = Number(budgetMonth);
            if (existing) {
            ...
            let shouldDeleteLinkedEntry = !transactionData.budgetCategory || transactionData.budgetRow == null || !amount;
            if (!shouldDeleteLinkedEntry && createBudgetEntry === false) {
              shouldDeleteLinkedEntry = window.confirm(
                'This transaction already contributes to Budget. Click OK to remove its linked budget entry, or Cancel to keep the existing budget entry unchanged.',
              );
            }
            if (shouldDeleteLinkedEntry) {
        ...
        if (isUpdate && originalAttachment && (recipientChanged || dateChanged) && !result?.moved) {
        ...
        if (isUpdate && result?.moved && result?.attachmentMoved) {
          successMessage = 'Transaction moved and attachment renamed.';
        } else if (isUpdate && result?.moved) {
          successMessage = `Transaction moved to ${result.month} ${result.year}.`;
        }
        // Settle-an-invoice link. Sent only when something about it changed,
        // because the server rewrites the invoice workbook on every call.
        const previousInvoiceNumber = originalTx?.invoiceNumber || '';
        const invoiceChanged = (invoiceNumber || '') !== previousInvoiceNumber;
        // A date edit on a still-linked transaction must move the invoice's
        // payment date with it, or the invoice stays paid on the old date.
        if ((invoiceChanged || (invoiceNumber && dateChanged)) && txRow != null) {
          try {
            // The invoice's own year: from the picked entry, or carried over
            // from the existing link when re-saving an already-paid one.
            const invoiceYear = findInvoiceByNumber(openInvoices, invoiceNumber)?.year
              || originalTx?.invoiceYear
              || result?.year
              || globalYear;
            await setTransactionInvoice(
              result?.year || globalYear,
              result?.month || txMonthName,
              txRow,
              invoiceNumber || null,
              invoiceNumber ? invoiceYear : null,
            );
            successMessage = invoiceNumber
              ? `${isUpdate ? 'Transaction updated' : 'Transaction added'} — invoice ${invoiceNumber} marked paid.`
              : `${isUpdate ? 'Transaction updated' : 'Transaction added'} — invoice link removed.`;
            // An unlinked invoice becomes payable again, so the picker must refresh.
            loadOpenInvoices().catch(() => {});
          } catch (err) {
            pushToast('error', 'Transaction saved but the invoice link failed: ' + (err.message || ''));
          }
        }
        if (!isUpdate && attachmentPick && txRow != null) {
          try {
            ...
            if (attachResult?.mode === 'link') {
              successMessage = 'Transaction added and attachment linked.';
            } else if (attachResult?.attachment?.storageMode === 'external') {
              successMessage = 'Transaction added and attachment saved to external folder.';
            } else {
              successMessage = 'Transaction added with attachment.';
            }
          } catch (err) {
  ...
  const refreshElements = useCallback(() => (
    refreshElementSlices({ getElements, loadElements, setElements })
  ), [loadElements]);
  ...
  const finalActivity = useMemo(() => applyActivityFilters(activityLog, {
    activityType,
    activityUser,
    activityActionType,
    activityYear,
    ...
    activityDateFrom,
    activityDateTo,
    activityQuery: deferredActivityQuery,
    activitySort,
    activityCashFlowCat,
  ...
  return (
    <>
      ...
      >
                    ...
                    onSync={async () => {
                      try {
                        ...
                        const [cf, summary] = await Promise.all([
                          getCashFlow(globalYear),
                          getTransactionBudgetSummary(globalYear),
                        ]);
                        setCashFlow(cf);
                        setTxBudgetSummary(summary);
                        pushToast('success', 'Cash flow synced');
                      } catch (err) {
                        pushToast('error', 'Sync failed: ' + err.message);
                      }
                ...
                <div className="px-4 py-3 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2">
                  ...
                  <div className="flex items-center gap-2 flex-wrap w-full sm:w-auto sm:justify-end">
                    ...
                    <button
                      ...
                      className={BUTTON_GHOST}
                      title="Reconcile this month against a bank statement PDF"
                    >
        ...
        {section === 'invoices' && (
          <InvoicesView
            ...
            onMarkPaid={setInvoiceMarkPaid}
            onDelete={setInvoiceDelete}
            onAttach={handleAttachInvoice}
            onOpenAttachment={handleOpenInvoiceAttachment}
            onRemoveAttachment={handleRemoveInvoiceAttachment}
          />
        )}
      ...
      <TransactionImpactDialog
        ...
        budgetImpact={pendingTx?.budgetImpact}
        onBudgetEntryChange={(checked) => setPendingTx((prev) => prev ? {
          ...prev,
          data: { ...prev.data, createBudgetEntry: checked },
        } : prev)}
      ...
      <MarkPaidDialog
        open={!!invoiceMarkPaid}
        invoice={invoiceMarkPaid}
        onConfirm={confirmMarkPaidInvoice}
        onCancel={() => setInvoiceMarkPaid(null)}
        submitting={invoiceSubmitting}
      />
      ...
      <ConfirmDialog
        ...
        message={invoiceDelete ? `Delete invoice ${invoiceDelete.invoiceNumber} (${invoiceDelete.recipient})? This removes the row from the Excel report.` : ''}
        confirmLabel="Delete"
        onConfirm={confirmDeleteInvoice}
        onCancel={() => setInvoiceDelete(null)}
      />
