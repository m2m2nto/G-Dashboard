...
// The update source is per-installation config, not part of the source tree:
// `dashboard/release.config.json` (gitignored, see release.config.example.json).
// A clone with no such file simply never checks for updates.
function loadReleasesUrl() {
  const candidates = [
    process.resourcesPath && path.join(process.resourcesPath, 'app', 'release.config.json'),
  ...
  for (const file of candidates) {
    try {
      ...
      if (cfg.updateRepoOwner && cfg.updateRepoName) {
        return `https://api.github.com/repos/${cfg.updateRepoOwner}/${cfg.updateRepoName}/releases/latest`;
      }
    } catch {
...
class Updater {
  ...
  async checkForUpdates() {
    const releasesUrl = loadReleasesUrl();
    if (!releasesUrl) return null; // no update source configured — updates disabled

    try {
      const data = await this._httpsGet(releasesUrl);
