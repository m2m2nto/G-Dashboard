...
// ---------------------------------------------------------------------------
// Invoice report files (per-year, v2 manifest `invoiceFiles` map)
// ---------------------------------------------------------------------------

export function getInvoiceFile(year) {
  const y = String(year);
  const manifest = getManifest();
  const invFiles = (manifest && manifest.invoiceFiles) || {};
  if (invFiles[y]) return resolvePath(invFiles[y]);

  // Derive from an existing registered file by swapping the year in its path.
  const existingYears = Object.keys(invFiles).sort();
  if (existingYears.length > 0) {
    const refYear = existingYears[existingYears.length - 1];
    return resolvePath(invFiles[refYear]).replace(refYear, y);
  }

  // Last resort: conventional filename in the project dir.
  const dir = getProjectDir() || DEFAULT_DATA_DIR;
  return resolve(dir, `${y} Invoice Report.xlsx`);
}

export async function listInvoiceYears() {
  const manifest = getManifest();
  const invFiles = (manifest && manifest.invoiceFiles) || {};
  const years = [];
  for (const [year, relPath] of Object.entries(invFiles)) {
    try {
      await access(resolvePath(relPath));
      years.push(year);
    } catch {}
  }
  return years.sort().reverse();
...
export function registerInvoiceFile(year, filePath) {
  const projectDir = getProjectDir();
  if (!projectDir) return;
  const manifest = getManifest();
  if (!manifest) return;
  if (!manifest.invoiceFiles) manifest.invoiceFiles = {};
  manifest.invoiceFiles[String(year)] = toManifestPath(filePath);
  writeManifest(projectDir, manifest);
}
...
/** @type {readonly ('certo' | 'possibile' | 'ottimistico')[]} */
