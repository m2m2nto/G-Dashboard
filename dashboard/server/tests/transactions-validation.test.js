  ...
  const { cleaned, error } = validateTransactionPayload(
    {
      ...
      transaction: 'Test',
      inflow: 10,
      iban: 'lu41  9995 5555 1234 5678',
    },
    { partial: false }
  );

  assert.ifError(error);
  assert.equal(cleaned.iban, 'LU419995555512345678');
});
...
test('attachTransactionMetadata adds budget timestamp and attachment data to rows', () => {
  ...
  attachTransactionMetadata(rows, {
    month: 'APR',
    resolvedCategories: {
      12: { budgetCategory: 'Custom Budget', budgetRow: 44 },
      13: { budgetCategory: 'Revenue Budget', budgetRow: 22 },
    },
    timestamps: {
...
test('attachTransactionMetadata sets budgetMonth from the linked budget entry', () => {
  const rows = [
    { row: 12, transaction: 'ACME SRL' },
    { row: 13, transaction: 'Client X' },
  ];
  ...
  attachTransactionMetadata(rows, {
    ...
    timestamps: {},
    attachments: {},
    budgetMonths: { 'APR-12': 6 },
  });

  assert.equal(rows[0].budgetMonth, 6, 'row with a linked entry gets its budget month');
  assert.equal(rows[1].budgetMonth, undefined, 'row without a linked entry has no budget month');
});

test('attachTransactionMetadata keeps budgetMonth 0 (GEN) rather than dropping it', () => {
  const rows = [{ row: 5, transaction: 'ACME SRL' }];
  attachTransactionMetadata(rows, {
    month: 'GEN',
    resolvedCategories: {},
    timestamps: {},
    attachments: {},
    budgetMonths: { 'GEN-5': 0 },
  });
  assert.equal(rows[0].budgetMonth, 0);
});
