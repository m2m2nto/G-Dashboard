// @ts-check
import { readFileSync, writeFileSync, renameSync, mkdirSync, existsSync, cpSync, readdirSync } from 'fs';
...
export function writeManifest(dir, data) {
  ...
  // tmp + rename: a crash mid-write must not truncate the manifest, since a
  // corrupt gl-project.json makes the server fail at startup.
  const tmp = `${p}.tmp`;
